...
def extract_transit_time(mdom_list):
    ...
    for imdom in mdom_list[:]:
        ...
        for ichannel in channel_list:
            ifiles_this_channel = sorted(glob.glob(imdom+f"/m*_{ichannel}_*.json"))
            mu_list = []
            sigma_list = []
            hv_list = []
            tt_info_map = []
...
def get_transit_time_dict(mdom_list):
    ...
    print(f"Transit time dict saved to /Users/epaudel/research_ua/icecube/upgrade/timing_calibration/scripts/mdom_transit_time.json")
...
def main():
    ...
    deployed_device_list = get_device_list("87","mDOM",geometry_files) + get_device_list("88","mDOM",geometry_files) + get_device_list("89","mDOM",geometry_files) + get_device_list("90","mDOM",geometry_files) + get_device_list("91","mDOM",geometry_files) + get_device_list("92","mDOM",geometry_files)

    print(f"deployed device in string 87 {len(get_device_list('87','mDOM',geometry_files))}")
